...
import { useUpdateDaycareProfile } from "@/http/daycares/update-daycare-profile";
...
interface FormUpdateDaycareProps {
  session: Session;
}
...
export default function FormUpdateDaycare({ session }: FormUpdateDaycareProps) {
  const { data } = useGetOurDaycare(session.access_token);
  ...
  useEffect(() => {
    if (data?.data) {
      form.reset({
        name: data.data.name,
        opening_days: data.data.opening_days,
        opening_hours: format(
          new Date(`1970-01-01T${data.data.opening_hours}`),
          "HH:mm"
        ),
        closing_hours: format(
          new Date(`1970-01-01T${data.data.closing_hours}`),
          "HH:mm"
        ),
        ...
        facility_images:
          data.data.facility_images?.map((img) => img.image_url) ?? [],
        address: data.data.address,
        location: data.data.location,
        location_tracking: data.data.location_tracking,
        is_disability: data.data.is_disability,
        ...
        price_lists:
          data.data.price_lists?.map((item) => ({
            name: item.name ?? "",
      ...
      if (data.data.images) {
        setImagePreview(
          `${baseUrl.replace(/\/+$/, "")}/${data.data.images.replace(
            /^\/+/,
            ""
          )}`
        );
      ...
      if (data.data.facility_images) {
        const imageUrls = data.data.facility_images.map(
          (img) =>
            `${baseUrl.replace(/\/+$/, "")}/${img.image_url.replace(
              /^\/+/,
              ""
            )}`
        );
        setFacilityImagesPreview(imageUrls);
      }
    }
  }, [data, form]);
  ...
  useEffect(() => {
    if (data?.data) {
      ...
      if (lat && lng) {
        const newPosition = { lat: Number(lat), lng: Number(lng) };
        setMarkerPosition(newPosition);
        form.setValue("latitude", newPosition.lat);
        form.setValue("longitude", newPosition.lng);
      } else {
        // Jika tidak ada data lat/lng, gunakan lokasi pengguna
  ...
  const { mutate: addDaycareHandler, isPending } = useUpdateDaycareProfile({
            ...
            <div className="grid md:grid-cols-2 grid-cols-1 gap-8">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Atas Nama di Bank <span className="text-red-500">*</span>
                    </FormLabel>
                    <FormControl>
                      <Input
                        type="text"
              ...
              <div className="space-y-4">
                ...
                {fields.map((field, index) => (
                  <div className="flex items-end gap-4" key={field.id}>
                    <div className="grid md:grid-cols-4 grid-cols-1 gap-4 md:gap-6 w-full">
                      <FormField
                        control={form.control}
